...
export const checkTeacherScheduleConflict = async (teacherId, weekdays, excludeClassId, newClassStartDate) => {
            ...
            for (const newWeekday of weekdays) {
                if (weekday.week_day === newWeekday.week_day) {
                    ...
                    if ((newStartTime >= existingStartTime &&
                        ...
                        return {
                            hasConflict: true,
                            conflictDetails: `Xung đột với lớp ${classData.name} vào thứ ${weekday.week_day === 8 ? "Chủ nhật" : weekday.week_day} từ ${weekday.start_time} đến ${formatEndTime(weekday.start_time, weekday.hours)}`,
                        };
                    }
                }
            }
...
const formatEndTime = (startTime, hours) => {
