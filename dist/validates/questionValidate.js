...
export const createQuestionValidator = [
    ...
    body().custom((value, { req }) => {
        ...
        const hasQuestions = Object.keys(req.body).some((key) => key.startsWith("questions["));
        if (!hasQuestions) {
    ...
    body().custom((value, { req }) => {
        ...
        const questionIndices = new Set(questionKeys
        ...
        for (const index of questionIndices) {
            ...
            // Kiểm tra options
            const optionKeys = Object.keys(req.body).filter((key) => key.startsWith(`questions[${index}][option][`));
            ...
            if (!["A", "B", "C", "D"].includes(correctOption)) {
                throw new Error(`Question ${index} correct_option must be A, B, C, or D.`);
            }
            // Kiểm tra score
