...
export const uploadToCloudinary = async (fileUrl, folder) => {
...
export const uploadMiddleware = multer({
    ...
    fileFilter: (req, file, cb) => {
        ...
        const filetypes = /jpeg|jpg|png|mp3|wav|mpeg|audio\/mpeg|audio\/wav|xlsx|xls|application\/vnd\.openxmlformats-officedocument\.spreadsheetml\.sheet|application\/vnd\.ms-excel/;
...
export const ensureUploadDirForQuestion = async (req, res, next) => {
    ...
    try {
        ...
        if (question_id) {
            console.log("Debug - Processing update request for question_id:", question_id);
            ...
            if (question.group.part.examParts.length === 1) {
                ...
                examPart = question.group.part.examParts.find((ep) => ep.exam_id === Number(exam_id));
            }
            if (!examPart) {
            ...
            const sanitizedExamName = examPart.exam.name.replace(/[^a-zA-Z0-9]/g, "_");
            const sanitizedPartName = question.group.part.name.replace(/[^a-zA-Z0-9]/g, "_");
...
export const ensureUploadDirForExcel = async (req, res, next) => {
    try {
        // Create base uploads directory if it doesn't exist
        const baseUploadPath = path.join(process.cwd(), "uploads");
        await fs.mkdir(baseUploadPath, { recursive: true });
