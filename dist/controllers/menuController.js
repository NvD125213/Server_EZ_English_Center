...
async function checkCircularParent(parentId, targetId) {
...
async function getFullSlugPath(menuId) {
    if (!menuId)
        return "";
...
const getMenuWithChildren = async (menuId, status, search, sortOrder = "asc") => {
    ...
    const menusWithChildren = await Promise.all(children.map(async (menu) => {
        const subChildren = await getMenuWithChildren(menu.id, status, search, sortOrder);
        return {
            ...menu,
...
export const MenuController = {
    get: async (req, res) => {
        try {
            ...
            const menusWithChildren = await Promise.all(rootMenus.map(async (menu) => {
                const children = await getMenuWithChildren(menu.id, status, search, "asc");
                return {
                    ...menu,
                    children,
                };
    ...
    update: async (req, res) => {
        try {
            ...
            if (validatedData.name) {
                const parentSlug = await getFullSlugPath(validatedData.parent_id || existingMenu.parent_id);
            ...
            if (validatedData.parent_id) {
                ...
                const isCircular = await checkCircularParent(validatedData.parent_id, menuId);
                if (isCircular) {
                    return res.status(400).json({
