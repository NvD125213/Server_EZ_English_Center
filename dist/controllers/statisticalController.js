...
export const StatisticalController = {
    getAllUserStatistical: async (req, res) => {
        try {
    ...
    getAllUserByMonth: async (req, res) => {
        try {
            const now = new Date();
    ...
    getPaymentStatistical: async (req, res) => {
        try {
            const now = new Date();
            const currentMonth = now.getMonth();
            const currentYear = now.getFullYear();
            ...
            const previousMonthTotal = Number(previousMonthPayments._sum?.amount || 0);
            // Calculate percentage change
            let percentageChange = 0;
    ...
    getPaymentStatisticalYear: async (req, res) => {
        try {
            const now = new Date();
            const currentYear = now.getFullYear();
    ...
    getCoursedFavorite: async (req, res) => {
        try {
            ...
            const coursesWithPaymentCount = await Promise.all(courses.map(async (course) => {
                const menu = await prisma.menu.findUnique({
                    where: { id: course.menu_id },
                    select: { name: true },
                });
                const totalPayments = course.classes.reduce((total, cls) => total + cls._count.payments, 0);
                return {
                    id: course.id,
