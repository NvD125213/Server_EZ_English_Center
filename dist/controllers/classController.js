...
const baseClassSchema = z.object({
    ...
    class_weekdays: z.array(z.object({
...
const createClassSchema = baseClassSchema.refine((data) => {
...
const updateClassSchema = baseClassSchema.partial().refine((data) => {
...
export const ClassController = {
    get: async (req, res) => {
        try {
            ...
            const { page, limit, sort_by, sort_order, search, teacher_id } = validatedQuery;
    ...
    create: async (req, res) => {
        try {
            ...
            const conflictCheck = await checkTeacherScheduleConflict(validatedData.teacher_id, weekdays.map((w) => ({
                week_day: w.week_day,
                hours: w.hours,
                start_time: w.start_time,
            })), undefined, new Date(validatedData.start_date));
            ...
            if (validatedData.teacher_id) {
                ...
                if (validatedData.class_weekdays) {
                    ...
                    const conflictCheck = await checkTeacherScheduleConflict(validatedData.teacher_id, weekdays.map((w) => ({
                        ...
                        hours: w.hours,
                        start_time: w.start_time,
                    })), classId, validatedData.start_date
    ...
    getStudentsByClassId: async (req, res) => {
        try {
            ...
            const formattedStudents = students.map((student) => {
                const studentPayment = student.class.payments.find((payment) => payment.student_id === student.student_id);
                return {
                    ...student,
    ...
    getListClassByAddressAndMonth: async (req, res) => {
        try {
    ...
    registerClass: async (req, res) => {
        try {
            const { class_id, staff_id, course_id, payment_method, student } = req.body;
            ...
            const result = await prisma.$transaction(async (tx) => {
                ...
                if (!existingStudent) {
                    ...
                    if (existingStudent.user.email !== student.user_email) {
                        throw new Error("Email học viên không khớp với tài khoản đã tồn tại");
                    }
                ...
                if (payment_method === "BANKING") {
                    ...
                    const { url, vnp_TxnRef } = generatePaymentUrl({
                        ...
                        orderInfo: `Thanh toan khoa hoc ${classData.course.menu.name.replace(/[^a-zA-Z0-9\s]/g, "")}`,
            ...
            if (result.paymentMethod === "BANKING" && result.paymentUrl) {
                try {
                    await sendMailPayment(student.user_email, result.courseName, Number(courseData.price), result.paymentUrl);
                }
                catch (emailError) {
