...
export const HistoryController = {
    getExamHistory: async (req, res) => {
        ...
        try {
            ...
            const examHistory = answers.reduce((acc, answer) => {
                const submissionTime = answer.created_at.toISOString();
                const existingSubmission = acc.find((submission) => submission.submitted_at === submissionTime);
            ...
            examHistory.forEach((submission) => {
                submission.total_score = Math.round((submission.correct_answers / submission.total_questions) * 100);
            });
            // Sort exam history by submission time (newest first)
            examHistory.sort((a, b) => new Date(b.submitted_at).getTime() -
