...
export const PaymentController = {
    handlePaymentReturn: (async (req, res) => {
        ...
        const checkSum = createVNPayChecksum(vnp_Params, process.env.VNP_HASHSECRET);
        ...
        try {
            ...
            if (responseCode === "00") {
                ...
                try {
                    const classSchedules = payment.class.class_schedules.map((schedule) => ({
                        week_day: schedule.weekday.week_day,
                        hours: schedule.weekday.hours,
                        start_time: schedule.weekday.start_time,
                    }));
                    await sendPaymentConfirmation(payment.student.email, payment.class.course.menu.name, payment.class.name, payment.class.start_date, Number(payment.amount), classSchedules, {
