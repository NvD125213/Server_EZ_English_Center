...
export const AddressController = {
  ...
  create: async (req: Request, res: Response): Promise<any> => {
    try {
      ...
      if (!coords || !coords.lat || !coords.lng) {
        return res.status(400).json({
          error:
            "Không thể xác định tọa độ cho địa chỉ này. Vui lòng kiểm tra lại thông tin địa chỉ.",
        });
      }
  ...
  getDistricts: async (req: Request, res: Response): Promise<any> => {
    try {
      ...
      const response = await axios.get(
        `${LOCATION_API_BASE}/p/${code}?depth=2`,
        {
          headers: LOCATION_API_HEADERS,
          responseType: "json",
        }
  ...
  getWards: async (req: Request, res: Response): Promise<any> => {
    try {
      const { code } = req.params;
      const response = await axios.get(
        `${LOCATION_API_BASE}/d/${code}?depth=2`,
        {
          headers: LOCATION_API_HEADERS,
          responseType: "json",
        }
